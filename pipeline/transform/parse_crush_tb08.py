"""Parse TB08 CSV/XLSX files from bronze → silver parquet."""
...
def _find_tb08_files(year: int) -> list[Path]:
    """Find TB08 CSV or XLSX/XLS files in data/raw/crush_reports/{year}/."""
...
def _parse_brix_code(brix_code: object) -> float | None:
    """Parse brix_code to float.

    Examples:
        "024500" → 24.5
        "000100" / 100 → None (code for no brix)
    """
...
def _parse_xls_tb08(filepath: Path, year: int) -> pd.DataFrame | None:
    ...
    current_grape_type_code: int | None = None
    current_grape_type_name: str | None = None
    rows: list[dict] = []
    ...
    for idx in range(len(raw)):
        ...
        # Check for STATE TOTAL header (district 18)
        ...
        matched_type: tuple[int, str] | None = None
        ...
        is_summary = (
            "wtd." in cell0.lower()
            or "avg." in cell0.lower()
            or "total" in cell0.lower()
        )
        ...
        rows.append({
            ...
            "variety_code": None,  # assigned below via hash
    ...
    # Assign variety codes via hash (XLS files don't have codes)
...
def _parse_csv_tb08(filepath: Path) -> pd.DataFrame | None:
    """Parse flat CSV TB08 file with encoding fallback."""
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Normalize column names to snake_case, map variants to standard names."""
...
def parse_year(year: int) -> pd.DataFrame | None:
    ...
    Reads CSV or XLS/XLSX files, normalizes columns, casts types,
    parses brix codes, and adds crop_year.
    """
    ...
    # Clean variety_name and grape_type_name
...
def parse_all_crush(years: list[int]) -> tuple[int, int]:
    """Parse all years and save silver parquets.

    Saves per-year parquets to data/silver/{year}_tb08.parquet, plus combined
    crush_data.parquet (row_type_code==2) and crush_summary.parquet
    (row_type_code==3).

    Returns:
        Tuple of (total_rows, years_parsed).
    """
    print("=== Transform: Parsing TB08 crush data ===")
    ...
    if all_frames:
        ...
        # Split into data rows and summary rows
        data_rows = all_data[all_data["row_type_code"] == 2]
        summary_rows = all_data[all_data["row_type_code"] == 3]

        data_rows.to_parquet(SILVER_DIR / "crush_data.parquet", index=False)
        summary_rows.to_parquet(SILVER_DIR / "crush_summary.parquet", index=False)

        print(f"  crush_data.parquet: {len(data_rows)} rows")
        print(f"  crush_summary.parquet: {len(summary_rows)} rows")
